...
vector<int> GRASP::construction(float alpha, float *val, int *prize, float upper_Val, int lower_Val, float best_Val, int best_Prize)
{
	int ck = 0;
	int i;
	...
	for(i = 0; cl_gen.getTime() <= 180 && !ck; i++)
	{
		...
		for(int j = 0; j < size_Sol-1 && !emp; j++){
			...
			}else
			{
				...
				//Max prize 2 best_Prize
				if(op_Sols[j+1] == 0 && local_Val <= upper_Val && best_Prize <= local_Prize){ 
					j = size_Sol;
					ck = 1; 
				}else if(op_Sols[j+1] == 0 && (local_Val > upper_Val) )
				{
					j = size_Sol;
					local_Val = 0;
					local_Prize = 0;
